...
public class MiniMax {
    ...
    public MiniMax(Field field, int maximizingPlayer, Map<String, Boolean> fourInARowCache, Map<String, Transposition> transpositions) {
        this.field = field;
    ...
    public MiniMaxScore miniMax(int depth, int player) {
        return miniMax(depth, player, Integer.MIN_VALUE, Integer.MAX_VALUE);
    }

    private MiniMaxScore miniMax(int depth, int player, int alpha, int beta) {
        ...
        for (int move : moves) {
            ...
            int score = miniMax(depth - 1, otherPlayer(player), alpha, beta).score;
